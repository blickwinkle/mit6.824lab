...
func (ck *Clerk) Get(key string) string {
	...
	for {
		...
		if servers, ok := ck.config.Groups[gid]; ok {
			...
			leader, ok := ck.GidToLeader[gid]
			if !ok || leader >= len(servers) {
				leader = 0
			}
			for si, count := leader, 0; count < len(servers); count, si = count+1, (si+1)%len(servers) {
				srv := ck.make_end(servers[si])
				var reply GetReply
				ok := srv.Call("ShardKV.Get", &args, &reply)
				if ok && (reply.Err == OK || reply.Err == ErrNoKey) {
					ck.GidToLeader[gid] = si
					return reply.Value
				}
